class Twofish():
    ...
    rs = [['1', '10100100', '1010101', '10000111', '1011010', '1011000', '11011011', '10011110'],
          ['10100100', '1010110', '10000010', '11110011',
              '11110', '11000110', '1101000', '11100101'],
          ['01', '10100001', '11111100', '11000001',
              '1000111', '10101110', '111101', '11001'],
    ...
    def skey(self):
        ...
        for i in range(4):
            for j in range(8):
                temp += self.bin_to_dec(key[j]) * \
                    self.bin_to_dec(self.rs[i][j])
            new_key.append(temp)
    ...
    def g(self, X, skey):
        ...
        X = self.q(X[0], 0) + self.q(X[1], 1) + \
            self.q(X[2], 0) + self.q(X[3], 1)
        X = self.xor(X, skey[0])
        X = [X[i: i + 8] for i in range(0, len(X), 8)]
        X = self.q(X[0], 0) + self.q(X[1], 0) + \
            self.q(X[2], 1) + self.q(X[3], 1)
        X = self.xor(X, skey[1])
        X = [X[i: i + 8] for i in range(0, len(X), 8)]
        X = self.q(X[0], 1) + self.q(X[1], 0) + \
            self.q(X[2], 1) + self.q(X[3], 0)
        X = [X[i: i + 8] for i in range(0, len(X), 8)]
        X = self.MDS(X)
        return X
